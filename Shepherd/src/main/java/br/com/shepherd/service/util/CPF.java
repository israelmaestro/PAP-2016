...
/**
 * Validates a CPF (Cadastro de Pessoa F\u00edsica - Brazilian individual
 * taxpayer registry number).
 *
 * @author George Gastaldi
 * @author Victor Rezende dos Santos
 */
//@Pattern.List({	@Pattern(regexp = "([0-9]{3}[.]?[0-9]{3}[.]?[0-9]{3}-[0-9]{2})|([0-9]{11}|^$)"),
@Pattern.List({	@Pattern(regexp = "([0-9]{3}[.]?[0-9]{3}[.]?[0-9]{3}-[0-9]{2})|([0-9]{11})"),
				// XXX.XXX.XXX-XX where X is always the same digit are not a
				// valid CPFs, but all of them passes the mod check. Needs to be
				// singled out each one via regexp
...
@Mod11Check.List({	@Mod11Check(checkDigitIndex = 12, endIndex = 10, treatCheck10As = '0',
								ignoreNonDigitCharacters = true),
					@Mod11Check(checkDigitIndex = 13, endIndex = 12, treatCheck10As = '0',
								ignoreNonDigitCharacters = true) })
