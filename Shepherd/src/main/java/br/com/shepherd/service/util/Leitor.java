 ...
 * <br/>
 *
 * Alterado e complementado por José Augusto Martins Nieviadonski para os cursos
 * de Java ministrados em pós-graduação e cursos da empresa JAMN Cursos
 * Técnicos.
 *
 * <br/>
 * <br/>
 *
 * Permitido a cópia e uso dessa classe desde que mantido essa mensagem de
 * copyrigth. <br/>
 * <br/>
 *
...
public class Leitor{
	...
	private static byte ajustaByte(long pValor){
		if(pValor < Byte.MIN_VALUE){ return Byte.MIN_VALUE; }

		if(pValor > Byte.MAX_VALUE){ return Byte.MAX_VALUE; }

		return (byte) pValor;
	}

	private static short ajustaShort(long pValor){
		if(pValor < Short.MIN_VALUE){ return Short.MIN_VALUE; }

		if(pValor > Short.MAX_VALUE){ return Short.MAX_VALUE; }

		return (short) pValor;
	}

	private static int ajustaInt(long pValor){
		if(pValor < Integer.MIN_VALUE){ return Integer.MIN_VALUE; }

		if(pValor > Integer.MAX_VALUE){ return Integer.MAX_VALUE; }

		return (int) pValor;
	}

	private static float ajustaFloat(double pValor){
		if(pValor < Float.MIN_VALUE){ return Float.MIN_VALUE; }

		if(pValor > Float.MAX_VALUE){ return Float.MAX_VALUE; }
	...
	public static byte readByte(String pPrompt, long pValor){
		...
		while(true){
			try{
				...
				if(tLinha.equals("")){ return ajustaByte(pValor); }
	...
	public static short readShort(String pPrompt, long pValor){
		...
		while(true){
			try{
				...
				printPrompt(pPrompt);
				tLinha = readLine().trim();
				if(tLinha.equals("")){ return ajustaShort(pValor); }
	...
	public static int readInt(String pPrompt, long pValor){
		...
		while(true){
			try{
				...
				printPrompt(pPrompt);
				tLinha = readLine().trim();
				if(tLinha.equals("")){ return ajustaInt(pValor); }
	...
	public static long readLong(String pPrompt, long pValor){
		...
		while(true){
			try{
				...
				printPrompt(pPrompt);
				tLinha = readLine().trim();
				if(tLinha.equals("")){ return pValor; }
	...
	public static float readFloat(String pPrompt, double pValor){
		...
		while(true){
			try{
				...
				printPrompt(pPrompt);
				tLinha = readLine().trim();
				if(tLinha.equals("")){ return ajustaFloat(pValor); }
	...
	public static boolean readBoolean(String pPrompt, boolean pValor){
		...
		while(true){
			...
			tLinha = readLine().trim();
			if(tLinha.equals("")){ return pValor; }
			if(tLinha.equalsIgnoreCase("true")){ return true; }
			if(tLinha.equalsIgnoreCase("false")){ return false; }
			System.out
